...
int main() {
  ...
  "replace a running app on disk"_test = [] {
    ...
    expect(std::filesystem::exists(fixture_a_path)) << "Run `make -C tests/app prepare-fixtures` to create fixture A.";
    expect(std::filesystem::exists(fixture_b_path)) << "Run `make -C tests/app prepare-fixtures` to create fixture B.";
    ...
    expect(fixture_a_team_id.has_value()) << "Fixture A must have a valid non-ad-hoc code signature.";
    expect(fixture_b_team_id.has_value()) << "Fixture B must have a valid non-ad-hoc code signature.";
